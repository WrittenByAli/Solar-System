...
| `/reviews` | `Reviews` |
| `/review-queue` | `GradeSubmissions` — any authenticated member (guests get the upgrade prompt); self-review and duplicate-review protections are DB-enforced |
...
| `/deploy` | `DeployHub` — self-hosting guide: Coolify wizard, env vars, checklist (localStorage `sa-deploy-checklist`), architecture, troubleshooting. Replaced the old `/host-archive` + `/directory` pages; both old paths redirect here |
...
| `solar-deploy.css` | `/deploy` — DeployHub (`.deploy-*` / `.sa-deploy-page` classes) |
| `solar-map.css` | `/map` |
...
- **Archive entries:** seeded subjects + user submissions live in the Supabase `archive_entries` table (`supabase_schema.sql`, `supabase_seed.sql`). `ArchiveGrid` reads approved entries per hub; `/submit` inserts pending entries (or updates a draft/edit-target row); `/profile` and `/my-submissions` list the current user's submission history. `is_draft`/`draft_saved_at`/`deleted_at`/`layer_data` are live: draft autosave, edit-and-resubmit, and soft-delete all ship as of the submission-management feature — see the Submission lifecycle paragraph under Routing above.
